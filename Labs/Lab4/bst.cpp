...
//Lab4        
	...
	int BinarySearchTree<Comparable>::height ( BinaryNode<Comparable> *t ) const
 	{
		...
		{
			int temp = height(t-> left) + 1;
			int temp2 = height(t-> right) + 1;
			if(temp > temp2)
				return temp;
			else
				return temp2;
	...
	//Call to numLeaves from client -- Acts as stub for private which uses root ptr
	template <class Comparable>
	...
	//Call to isBalanced from client -- Acts as a stub for Private which uses Root ptr
	template <class Comparable>
	...
	//Call to isBalanced from private -- Accesses the Root prt
	template <class Comparable>
